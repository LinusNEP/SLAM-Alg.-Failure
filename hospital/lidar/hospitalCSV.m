...
gt_hospital_data = readtable(gt_hospital);
hector_hospital_data = readtable(hector_hospital);
gmap_hospital_data = readtable(gmap_hospital);
karto_hospital_data = readtable(karto_hospital);

% Extract the timestamp column
gt_time_stamp = gt_hospital_data{:,1};
hector_time_stamp = hector_hospital_data{:,1};
gmap_time_stamp = gmap_hospital_data{:,1};
karto_time_stamp = karto_hospital_data{:,1};
...
gt_x = gt_hospital_data{:,2};
gt_y = gt_hospital_data{:,3};
gt_z = gt_hospital_data{:,4};

hector_x = hector_hospital_data{:,2};
hector_y = hector_hospital_data{:,3};
hector_z = hector_hospital_data{:,4};

gmap_x = gmap_hospital_data{:,2};
gmap_y = gmap_hospital_data{:,3};
gmap_z = gmap_hospital_data{:,4};

karto_x = karto_hospital_data{:,2};
karto_y = karto_hospital_data{:,3};
karto_z = karto_hospital_data{:,4};

% Extract the qx, qy, qz, qw rotation columns
gt_qz = gt_hospital_data{:,7};
hector_qz = hector_hospital_data{:,7};
gmap_qz = gmap_hospital_data{:,7};
karto_qz = karto_hospital_data{:,7};

%   Rearrange the position and rotation data
%   Pose data
gt_translation_ts = [(gt_time), (gt_x )+0.015, (gt_y)-0.015, gt_z];
hector_translation_ts = [(hector_time), (hector_x), (hector_y), hector_z];
karto_translation_ts = [(karto_time), (karto_x)+0.015, (karto_y)-0.015, karto_z];
gmap_translation_ts = [(gmap_time), (gmap_x)+0.02, (gmap_y)-0.02, gmap_z];

%   Angular data 
gt_rotation_ts = [(gt_qz)+0.015];
hector_rotation_ts = [(hector_qz)];
karto_rotation_ts = [(karto_qz)+0.015];
gmap_rotation_ts = [(gmap_qz)+0.02];
...
%   target length of the interpolated matrix
    target_length_distance = 5443;
    interp_error_hector = interp1(1:length(error_hector), error_hector, linspace(1, length(error_hector), target_length_distance))';
    interp_error_gmap = interp1(1:length(error_gmap), error_gmap, linspace(1, length(error_gmap), target_length_distance))';
    interp_error_karto = interp1(1:length(error_karto), error_karto, linspace(1, length(error_karto), target_length_distance))';
...
%   Interpolate the scale drift time
    drift_time_hector = imresize(hector_time,[5443,1]);
    drift_time_karto = imresize(karto_time,[5443,1]);
    drift_time_gmap = imresize(gmap_time,[5443,1]);
