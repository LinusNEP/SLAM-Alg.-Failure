...
gt_hospital = rosbag('gtHospital.bag'); % GroundTruth
hector_hospital = rosbag('hectorHospital.bag');
gmapping_hospital = rosbag('gMapHospital.bag');
karto_hospital = rosbag('kartoHospital.bag');


%% ROS Topics

% GT
gt_tf = select(gt_hospital, 'Topic', '/tf');
gt_msgs = readMessages(gt_tf);

%Hector
hector_tf = select(hector_hospital, 'Topic', '/tf');
hector_msgs = readMessages(hector_tf);

%gmap
gmap_tf = select(gmapping_hospital, 'Topic', '/tf');
gmap_msgs = readMessages(gmap_tf);

%karto
karto_tf = select(karto_hospital, 'Topic', '/tf');
...
%{
gt_translation_ts = timeseries(gt_translation_data,gt_time_data);
hector_translation_ts = timeseries(hector_translation_data,hector_time_data);
gmap_translation_ts = timeseries(gmap_translation_data,gmap_time_data);
karto_translation_ts = timeseries(karto_translation_data,karto_time_data);
...
%}
gt_translation_ts = timeseries(gt_translation_data + 0.015,gt_time_data + 0.015);
hector_translation_ts = timeseries(hector_translation_data + 0.01,hector_time_data + 0.01);
gmap_translation_ts = timeseries(gmap_translation_data,gmap_time_data);
karto_translation_ts = timeseries(karto_translation_data + 0.002,karto_time_data + 0.002);


gt_rotation_ts = timeseries(gt_rotation_data + 0.015,gt_time_data + 0.015);
hector_rotation_ts = timeseries(hector_rotation_data + 0.01,hector_time_data + 0.01);
gmap_rotation_ts = timeseries(gmap_rotation_data,gmap_time_data);
karto_rotation_ts = timeseries(karto_rotation_data + 0.002,karto_time_data+ 0.002);
...
plot(gt_translation_ts.Data(:,1),gt_translation_ts.Data(:,2),"r","LineWidth",1.0)
hold on
plot(hector_translation_ts.Data(:,1),hector_translation_ts.Data(:,2),"b","LineWidth",1.0)
hold on
...
ylabel('y-pose [m]')
%zlabel('z-pose [m]')
legend("GTHospital","hectorHospital","kartoHospital", "gmappingHospital")
%}
...
% GT
interp_gt_x = imresize(ground_truth_x,[5482,1]);
interp_gt_y = imresize(ground_truth_y,[5482,1]);
interp_gt_theta = imresize(ground_truth_theta,[5482,1]);
interp_gt_time = imresize(gt_time,[5482,1]);

% Hector-SLAM
interp_hector_x = imresize(hector_estimated_x,[5482,1]);
interp_hector_y = imresize(hector_estimated_y,[5482,1]);
interp_hector_theta = imresize(hector_estimated_theta,[5482,1]);
interp_hector_time = imresize(hector_time,[5482,1]);

% Karto-SLAM
interp_karto_x = imresize(karto_estimated_x,[5482,1]);
interp_karto_y = imresize(karto_estimated_y,[5482,1]);
interp_karto_theta = imresize(karto_estimated_theta,[5482,1]);
interp_karto_time = imresize(karto_time,[5482,1]);

% Gmapping
interp_gmapping_x = imresize(gmap_estimated_x,[5482,1]);
interp_gmapping_y = imresize(gmap_estimated_y,[5482,1]);
interp_gmapping_theta = imresize(gmap_estimated_theta,[5482,1]);
interp_gmap_time = imresize(gmapping_time,[5482,1]);
...
xline(mean(hector_ate),'--k', '\mu','HandleVisibility','off');
xline(mean(karto_ate),'--k', '\mu','HandleVisibility','off');
xline(mean(gmapping_ate),'--k', '\mu','HandleVisibility','off');
...
drift_time_hector = imresize(hector_time,[5481,1]);
drift_time_karto = imresize(karto_time,[5481,1]);
drift_time_gmap = imresize(gmapping_time,[5481,1]);
...
estimated_posesInt = imresize(estimated_poses(:,1),[5481,1]);
...
interp_error_hector = imresize(error_hector,[5481,1]);
interp_error_gmap = imresize(error_gmap,[5481,1]);
interp_error_karto = imresize(error_karto,[5481,1]);
...
disp(['Time to convergence: ', num2str(hector_time_to_convergence), 's']);

% Plot the ATE and TTC
figure;
stem(interp_gt_time, hector_ate, 'LineWidth', 1.0);
hold on
stem(interp_gt_time, karto_ate, 'LineWidth', 1.0);
hold on
stem(interp_gt_time, gmapping_ate, 'LineWidth', 1.0);
