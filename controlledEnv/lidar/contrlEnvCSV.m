...
%                   Controlled Environment (Contld Env.)
...
%   Load the data to workspace    
    gt_CtrlEnv = 'gtctrldEnv.csv'; 
    hector_CtrlEnv = 'hectorCtrlEnv.csv';
    gmap_CtrlEnv = 'gmapCtrlEnv.csv';
    karto_CtrlEnv = 'kartoCtrlEnv.csv';

%   Read and extract the messages 
    gt_CtrlEnv_data = readtable(gt_CtrlEnv);
    hector_CtrlEnv_data = readtable(hector_CtrlEnv);
    gmap_CtrlEnv_data = readtable(gmap_CtrlEnv);
    karto_CtrlEnv_data = readtable(karto_CtrlEnv);

%   Extract the timestamp column
    gt_time_stamp = gt_CtrlEnv_data{:,1};
    hector_time_stamp = hector_CtrlEnv_data{:,1};
    gmap_time_stamp = gmap_CtrlEnv_data{:,1};
    karto_time_stamp = karto_CtrlEnv_data{:,1};
...
%   Extract the x, y, z position columns
    gt_x = gt_CtrlEnv_data{:,2};
    gt_y = gt_CtrlEnv_data{:,3};
    gt_z = gt_CtrlEnv_data{:,4};

    hector_x = hector_CtrlEnv_data{:,2};
    hector_y = hector_CtrlEnv_data{:,3};
    hector_z = hector_CtrlEnv_data{:,4};

    gmap_x = gmap_CtrlEnv_data{:,2};
    gmap_y = gmap_CtrlEnv_data{:,3};
    gmap_z = gmap_CtrlEnv_data{:,4};

    karto_x = karto_CtrlEnv_data{:,2};
    karto_y = karto_CtrlEnv_data{:,3};
    karto_z = karto_CtrlEnv_data{:,4};

%   Extract the qx, qy, qz, qw rotation columns
    gt_qz = gt_CtrlEnv_data{:,7};
    hector_qz = hector_CtrlEnv_data{:,7};
    gmap_qz = gmap_CtrlEnv_data{:,7};
    karto_qz = karto_CtrlEnv_data{:,7};
...
%   Pose data
    ...
    gmap_translation_ts = [gmap_time, (gmap_x)+ 0.01, (gmap_y)- 0.01, gmap_z];
...
%   Interpolate the error to match the distance traveled
    interp_error_hector = imresize(error_hector,[961,1]);
    interp_error_gmap = imresize(error_gmap,[961,1]);
    interp_error_karto = imresize(error_karto,[961,1]);   
...
%   Create an array of ATE values for multiple runs of the slam algorithms
    ate_values_hector = [hector_ate];
    ate_values_gmap = [gmapping_ate];
    ate_values_karto = [karto_ate];

%   Set the bin edges for the error histogram plot
...
%   Interpolate the scale drift time
    drift_time_hector = imresize(hector_time,[961,1]);
    drift_time_karto = imresize(karto_time,[961,1]);
    drift_time_gmap = imresize(gmap_time,[961,1]);                
