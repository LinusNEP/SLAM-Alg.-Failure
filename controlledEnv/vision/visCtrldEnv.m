...
%                  Controlled Environment (Ctrld Env.)
...
gtVisCtrldEnv = 'gtVisCtrldEnv.csv'; % GroundTruth
rtabCtrldEnv = 'rtabCtrldEnv.csv';
orbslamCtrldEnv = 'orbslamCtrldEnv.csv';

%   Read and extract the messages 

gtVisCtrldEnv_data = readtable(gtVisCtrldEnv);
rtabCtrldEnv_data = readtable(rtabCtrldEnv);
orbslamCtrldEnv_data = readtable(orbslamCtrldEnv);

% Extract the timestamp column
gt_time_stamp = gtVisCtrldEnv_data{:,1};
rtab_time_stamp = rtabCtrldEnv_data{:,1};
orbslam_time_stamp = orbslamCtrldEnv_data{:,1};
...
gt_x = gtVisCtrldEnv_data{:,2};
gt_y = gtVisCtrldEnv_data{:,3};
gt_z = gtVisCtrldEnv_data{:,4};

rtab_x = rtabCtrldEnv_data{:,2};
rtab_y = rtabCtrldEnv_data{:,3};
rtab_z = rtabCtrldEnv_data{:,4};

orbslam_x = orbslamCtrldEnv_data{:,2};
orbslam_y = orbslamCtrldEnv_data{:,3};
orbslam_z = orbslamCtrldEnv_data{:,4};

% Extract the qx, qy, qz, qw rotation columns
gt_qz = gtVisCtrldEnv_data{:,7};
rtab_qz = rtabCtrldEnv_data{:,7};
orbslam_qz = orbslamCtrldEnv_data{:,7};
...
disp('The standard deviation of the scale drift for RTAB and ORB-SLAM are:')
disp([std(scale_drift_rtab) std(scale_drift_orbslam)])
